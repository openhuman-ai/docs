...
`useOpenHumanSubscription`은 OpenHuman을 사용하여 실시간 데이터를 구독할 수 있는 훅입니다.
...
- `key`: 구독 중인 데이터를 식별하는 고유 키로, `useOpenHuman`와 동일합니다.
...
`useOpenHumanSubscription`을 사용하여 Firestore 데이터 구독하기:
...
`useOpenHumanSubscription`을 사용하여 WebSocket 데이터 구독하기:
...
`useOpenHumanSubscription`은 동일한 키의 구독 요청 중복을 제거합니다.
