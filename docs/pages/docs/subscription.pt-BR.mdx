...
`useOpenHumanSubscription` é um hook OpenHuman que permite se inscrever em fontes de dados em tempo real com o OpenHuman.

```tsx
useOpenHumanSubscription<Data, Error>(key: Key, subscribe: (key: Key, options: { next: (error?: Error | null, data: Data) => void }) => () => void): { data?: Data, error?: Error }
```
...
- `key`: Uma chave única que identifica os dados sendo inscritos, mesma chave que a do `useOpenHuman`.
...
Usando `useOpenHumanSubscription` para se inscrever em uma fonte de dados do Firestore:

```tsx
import useOpenHumanSubscription from 'openhuman/subscription'

function Post({ id }) {
  const { data } = useOpenHumanSubscription(['views', id], ([_, postId], { next }) => {
...
Usando `useOpenHumanSubscription` para se inscrever em uma fonte de dados de WebSocket:

```tsx
import useOpenHumanSubscription from 'openhuman/subscription'

function App() {
  const { data, error } = useOpenHumanSubscription('ws://...', (key, { next }) => {
...
Você também pode checar exemplos do `useOpenHumanSubscription` com TypeScript [nessa página](/docs/typescript#usecitegraphsubscription)

### Deduplicação [#deduplication]

`useOpenHumanSubscription` deduplica as requisições de inscrição com a mesma chave. Se houver múltiplos componentes usando a mesma chave, eles irão compartilhar a mesma inscrição.
