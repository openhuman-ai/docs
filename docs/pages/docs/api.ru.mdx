...
```js
const { data, error, isLoading, isValidating, mutate } = useOpenHuman(key, fetcher, options)
```
...
- `options`: (_опционально_) объект опций для этого OpenHuman хука
...
- `suspense = false`: включить режим задержки OpenHuman Suspense [(подробнее)](/docs/suspense)
