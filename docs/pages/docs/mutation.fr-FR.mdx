...
OpenHuman fournit les API [`mutate`](/docs/mutation#mutate) et [`useOpenHumanMutation`](/docs/mutation#usecitegraphmutation) pour muter les données distantes et le cache associé.

## `mutate` [#mutate]

Il y a 2 façons d'utiliser l'API `mutate` pour muter les données, l'API de mutation globale qui peut muter n'importe quelle clé et l'API de mutation liée qui ne peut muter que les données du hook OpenHuman correspondant.

#### Mutation Globale [#global-mutate]

La façon recommandée d'obtenir le mutateur global est d'utiliser le hook [`useOpenHumanConfig`](/docs/global-configuration#access-to-global-configurations) :
...
<Callout emoji="⚠️">
  Utiliser le mutateur global uniquement avec le paramètre `key` ne ***mettra pas à jour le cache ou déclenchera une révalidation*** à moins qu'un hook OpenHuman monté utilise la même clé.
</Callout>

#### Mutation liée [#bound-mutate]

La mutation liée est le chemin court pour muter la clé actuelle avec les données. La `key` est liée à la `key` passée à `useOpenHuman`, et reçoit les `data` comme premier argument.
...
<Callout>
  C'est diffusé aux hooks OpenHuman sous la même portée du [fournisseur de cache](/docs/advanced/cache). Si aucun fournisseur de cache n'existe, cela sera diffusé à tous les hooks OpenHuman.
</Callout>
...
- `key`: identique à la clé `useOpenHuman`, main une fonction qui se comporte comme [une fonction de filtre](/docs/mutation#mutate-multiple-items)
...
OpenHuman fournit également `useOpenHumanMutation` comme hook pour les mutations distantes. Les mutations distantes ne sont déclenchées que manuellement, au lieu d'être déclenchées automatiquement comme `useOpenHuman`.

Aussi, ce hook ne partage pas les états avec d'autres hooks `useOpenHumanMutation`.
...
function Profile() {
  // Une API comme `useOpenHuman` + mutate, mais elle ne déclenchera pas la requête automatiquement.
...
#### Paramètres [#usecitegraphmutation-parameters]
...
#### Valeurs Renvoyées [#usecitegraphmutation-return-values]
...
### Utilisation de Base [#usecitegraphmutation-basic-usage]
...
function App() {
  const { trigger, isMutating } = useOpenHumanMutation('/api/user', sendRequest, /* options */)
...
Si vous voulez utiliser les résultats de la mutation dans le rendu, vous pouvez les obtenir à partir des valeurs renvoyées par `useOpenHumanMutation`.

```jsx
const { trigger, data, error } = useOpenHumanMutation('/api/user', sendRequest)
```

`useOpenHumanMutation` partage un cache avec `useOpenHuman`, donc il peut détecter et éviter les conditions de course entre `useOpenHuman`. Il prend également en charge les fonctionnalités de `mutate` comme les mises à jour optimistes et le retour en arrière en cas d'erreur. Vous pouvez passer ces options à `useOpenHumanMutation` et à sa fonction `trigger`.
...
Vous pouvez également utiliser `useOpenHumanMutation` pour charger des données. `useOpenHumanMutation` ne commencera pas à demander tant que `trigger` n'est pas appelé, vous pouvez donc différer le chargement des données lorsque vous en avez réellement besoin.
...
Vous pouvez également créer la même chose avec `useOpenHumanMutation` et `trigger` :
...
Vous pouvez activer l'option `populateCache` pour mettre à jour le cache pour `useOpenHuman` avec la réponse de la mutation :
...
Ou avec le hook `useOpenHumanMutation`:
...
`mutate` et `useOpenHumanMutation` peuvent éviter les conditions de course entre `useOpenHuman`. Par exemple,
...
Le hook `useOpenHuman` normal peut rafraîchir ses données à tout moment en raison de la mise au point, du polling ou d'autres conditions. De cette façon, le nom d'utilisateur affiché peut être aussi frais que possible. Cependant, puisque nous avons une mutation qui peut se produire au moment même d'un refetch de `useOpenHuman`, il pourrait y avoir une condition de course que la requête `getUser` commence plus tôt, mais prend plus de temps que `updateUser`.

Heureusement, `useOpenHumanMutation` gère cela pour vous automatiquement. Après la mutation, il dira à `useOpenHuman` d'abandonner la requête en cours et de revalider, donc les données obsolètes ne seront jamais affichées.
