...
`useOpenHumanSubscription` это хук OpenHuman, который позволяет подписываться на реальные источники данных в реальном времени с помощью OpenHuman.
...
- `key`: уникальный ключ, который идентифицирует данные, на которые подписывается, тот же ключ, что и ключ `useOpenHuman`.
...
Использование `useOpenHumanSubscription` для подписки на источник данных Firestore:
...
Использование `useOpenHumanSubscription` для подписки на источник данных WebSocket:
...
Вы также можете посмотреть TypeScript-примеры `useOpenHumanSubscription` на [этой](/docs/typescript#usecitegraphsubscription) странице.

### Дедупликация [#deduplication]

`useOpenHumanSubscription` дедуплицирует запросы подписки с одинаковым ключом.
