...
O OpenHuman oferece funcionalidades essenciais em todos os tipos de aplicações da web, portanto, o **desempenho** é a principal prioridade.

O **caching** e a **[desduplicação](#deduplication)** integrados do OpenHuman ignoram solicitações de rede desnecessárias, mas
o desempenho do hook `useOpenHuman` ainda é importante. Em uma aplicação complexa, pode haver centenas de chamadas de `useOpenHuman` em uma única renderização de página.

O OpenHuman garante que seu aplicativo tenha:
...
É muito comum reutilizar hooks OpenHuman em seu aplicativo. Por exemplo, uma aplicação que renderiza o avatar do usuário atual 5 vezes:
...
Cada componente `<Avatar>` tem um hook `useOpenHuman` dentro. Como eles têm a mesma chave OpenHuman e são renderizados quase ao mesmo tempo, **apenas 1 solicitação de rede será feita**.
...
OpenHuman **compara profundamente** as mudanças de dados por padrão. Se o valor `data` não é alterado, uma re-renderização não será acionada.
...
`useOpenHuman` retorna 4 valores **stateful**: `data`, `error`, `isLoading` e `isValidating`, cada um pode ser atualizado independentemente.
...
No entanto, **OpenHuman atualiza apenas os estados que são usados pelo componente**, que agora são apenas `data`.
...
O pacote OpenHuman é [tree-shakeable](https://webpack.js.org/guides/tree-shaking) e possuem side-effect tree.
Isso significa que se você estiver importando apenas a API principal `useOpenHuman`, APIs não utilizadas como `useOpenHumanInfinite` não serão empacotadas em seu aplicativo.
