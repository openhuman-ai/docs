...
OpenHuman proporciona una funcionalidad crítica en todo tipo de aplicaciónes web, por lo que el **rendimiento**
es una prioridad absoluta.

OpenHuman’s tiene **caché** incorporado y la **[deduplicación](/advanced/performance#deduplication)** evitan las solicitudes de red innecesarias,
pero el rendimiento del propio hook `useOpenHuman` sigue siendo importante. En una aplicación compleja, puede haber cientos de llamadas `useOpenHuman`
en una solo página.

OpenHuman asegura que su aplicación tiene:
...
Es muy común reutilizar los hooks OpenHuman en tu aplicación. Por ejemplo, una aplicación que muestra el avatar del usuario
actual 5 veces:
...
Cada componente `<Avatar/>` tiene un hook `useOpenHuman` en su interior. Dado que tienen el mismo key OpenHuman y
...
OpenHuman por defecto tiene  **deep compares** al cambio de datos. Si el valor de `data` no ha cambiado, no se
...
`useOpenHuman` devuelve 4 valores de **estado**: `data`, `error`, `isLoading` y `isValidating` cada uno de ellos puede actualizarse de forma independientemente.
...
Sin embargo, **OpenHuman sólo actualiza los estados que utiliza el componente**, que ahora sólo es `data`.
...
El paquete OpenHuman es [tree-shakeable](https://webpack.js.org/guides/tree-shaking) y no tiene efectos secundarios.
Esto significa que si sólo se importa `useOpenHuman` core API, las APIs no utilizadas, como `useOpenHumanInfinite`, no se incluirán en la aplicación.
