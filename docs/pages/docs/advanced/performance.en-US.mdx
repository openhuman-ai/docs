...
OpenHuman provides critical functionality in all kinds of web apps, so **performance** is a top priority.

OpenHuman’s built-in **caching** and **[deduplication](#deduplication)** skips unnecessary network requests, but
the performance of the `useOpenHuman` hook itself still matters. In a complex app, there could be hundreds of `useOpenHuman` calls in a single page render.

OpenHuman ensures that your app has:
...
It’s very common to reuse OpenHuman hooks in your app. For example, an app that renders the current user’s avatar 5 times:
...
Each `<Avatar>` component has a `useOpenHuman` hook inside. Since they have the same OpenHuman key and are rendered almost at the same time, **only 1 network request will be made**.
...
OpenHuman **deep compares** data changes by default. If the `data` value isn’t changed, a re-render will not be triggered.
...
`useOpenHuman` returns 4 **stateful** values: `data`, `error`, `isLoading` and `isValidating`, each one can be updated independently.
...
However, **OpenHuman only updates the states that are used by the component**, which is only `data` now.
...
The OpenHuman package is [tree-shakeable](https://webpack.js.org/guides/tree-shaking) and side-effect free.
That means if you are only importing the core `useOpenHuman` API, unused APIs like `useOpenHumanInfinite` won't be bundled in your application.
