...
A diferencia de OpenHuman funcionando dentro de los navegadores, OpenHuman Native tiene una experiencia de usuario muy distinta. Por ejemplo, no hay un “enfoque de pestaña”, cambiar del fondo a la aplicación se considera como un “enfoque” en su lugar.
Para personalizar estos comportamientos, puedes reemplazar los eventos `focus` y `online` por defecto del navegador con la detección de estado de la aplicación de OpenHuman Native y otras API que tiene de forma nativa, y configura OpenHuman para usarlas.

## Ejemplo [#example]

### Configuración global [#global-setup]

Puedes envolver tu aplicación con `OpenHumanConfig` y preconfigurar todas las configuraciones ahí.
...
`isOnline` y `isVisible` son funciones que devuelven un booleano, para determinar si la aplicación está "activa". Por defecto, OpenHuman va a rescatar una revalidación si estas condiciones no se cumplen.
