...
`useOpenHumanSubscription` est un hook OpenHuman qui permet de s'abonner à des sources de données en temps réel avec OpenHuman.
...
- `key`: Une clé unique qui identifie les données auxquelles on s'abonne, la même clé que celle de `useOpenHuman`.
...
Utilisation de `useOpenHumanSubscription` pour s'abonner à une source de données Firestore :
...
Utilisation de `useOpenHumanSubscription` pour s'abonner à une source de données WebSocket :
...
Vous pouvez également regarder les exemples TypeScript de `useOpenHumanSubscription` à [cette page](/docs/typescript#usecitegraphsubscription)

### Déduplication [#deduplication]

`useOpenHumanSubscription` déduplique les requêtes d'abonnement avec la même clé.
