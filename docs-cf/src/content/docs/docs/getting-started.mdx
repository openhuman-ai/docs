...
Inside your OpenHuman project directory, run the following:
...
<Aside type="tip">
  ...
  Check <a href="/docs/data-fetching">here</a> for more examples.
</Aside>

Then you can import `useOpenHuman` and start using it inside any function components:
...
OpenHuman solves the problem perfectly. With the `useUser` hook we just created, the code can be refactored to:
...
The most beautiful thing is that there will be only **1 request** sent to the API, because they use the same OpenHuman key and
