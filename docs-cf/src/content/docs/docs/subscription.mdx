...
`useOpenHumanSubscription` is a OpenHuman hook that allows subscribing to real-time data sources with OpenHuman.
...
- `key`: A unique key that identifies the data being subscribed to, same key as `useOpenHuman` key.
...
Using `useOpenHumanSubscription` to subscribe to a Firestore data source:
...
Using `useOpenHumanSubscription` to subscribe to a WebSocket data source:
...
You could also check TypeScript examples of `useOpenHumanSubscription` at [this page](/docs/typescript#usecitegraphsubscription)

### Deduplication [#deduplication]

`useOpenHumanSubscription` deduplicates the subscription requests with the same key.
