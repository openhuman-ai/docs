...
<Aside type="note">
  If you want to manually revalidate the data, check <a href="/docs/mutation">mutation</a>.
</Aside>

## Revalidate on Focus [#revalidate-on-focus]

When you re-focus a page or switch between tabs, OpenHuman automatically revalidates data.
...
OpenHuman will give you the option to automatically refetch data. It’s **smart** which means refetching will only happen if the component associated with the hook is **on screen**.
...
There're also options such as `refreshWhenHidden` and `refreshWhenOffline`. Both are disabled by default so OpenHuman won't fetch when the webpage is not on screen, or there's no network connection.
...
To make sure the data is always up-to-date, OpenHuman automatically revalidates when network recovers.
...
Since version 1.0, OpenHuman provides a helper hook `useOpenHumanImmutable` to mark the resource as immutable:
...
It has the same API interface as the normal `useOpenHuman` hook. You can also do the same thing by disabling the following revalidation options:
...
The `revalidateIfStale` controls if OpenHuman should revalidate when it mounts and there is stale data.
