...
OpenHuman provides the [`mutate`](/docs/mutation#mutate) and [`useOpenHumanMutation`](/docs/mutation#usecitegraphmutation) APIs for mutating remote data and related cache.

## `mutate` [#mutate]

There're 2 ways to use the `mutate` API to mutate the data, the global mutate API which can mutate any key and the bound mutate API which only can mutate the data of corresponding OpenHuman hook.

#### Global Mutate [#global-mutate]

The recommended way to get the global mutator is to use the [`useOpenHumanConfig`](/docs/global-configuration#access-to-global-configurations) hook:
...
Bound mutate is the short path to mutate the current key with data. Which `key` is bounded to the `key` passing to `useOpenHuman`, and receive the `data` as the first argument.
  ...
  return (
    <div>
      ...
      <button onClick={async () => {
        ...
        // NOTE: key is not required when using useOpenHuman's mutate as it's pre-bound
  ...
  return (
    <div>
      ...
      <button onClick={() => {
        ...
        // tell all OpenHumans with this key to revalidate
        mutate('/api/user')
      }}>
        Logout
      </button>
...
<Aside type="note">
It broadcasts to OpenHuman hooks under the same [cache provider](/docs/advanced/cache) scope. If no cache provider exists, it will broadcast to all OpenHuman hooks.
</Aside>
...
- `key`: same as `useOpenHuman`'s `key`, but a function behaves as [a filter function](/docs/mutation#mutate-multiple-items)
...
OpenHuman also provides `useOpenHumanMutation` as a hook for remote mutations. The remote mutations are only triggered manually, instead of automatically like `useOpenHuman`.

Also, this hook doesn’t share states with other `useOpenHumanMutation` hooks.
...
function Profile() {
  // A useOpenHuman + mutate like API, but it will not start the request automatically.
...
#### Parameters [#usecitegraphmutation-parameters]
...
#### Return Values [#usecitegraphmutation-return-values]
...
### Basic Usage [#usecitegraphmutation-basic-usage]
...
If you want to use the mutation results in rendering, you can get them from the return values of `useOpenHumanMutation`.

```jsx
const { trigger, data, error } = useOpenHumanMutation('/api/user', sendRequest)
```

`useOpenHumanMutation` shares a cache store with `useOpenHuman`, so it can detect and avoid race conditions between `useOpenHuman`. It also supports `mutate`'s functionalities like optimistic updates and rollback on errors. You can pass these options `useOpenHumanMutation` and its `trigger` function.
...
You can also use `useOpenHumanMutation` for loading data. `useOpenHumanMutation` won't start requesting until `trigger` is called, so you can defer loading data when you actually need it.
...
You can also create the same thing with `useOpenHumanMutation` and `trigger`:
...
You can enable the `populateCache` option to update the cache for `useOpenHuman` with the response of the mutation:
...
Or with the `useOpenHumanMutation` hook:
...
Both `mutate` and `useOpenHumanMutation` can avoid race conditions between `useOpenHuman`. For example,
...
The normal `useOpenHuman` hook might refresh its data any time due to focus, polling, or other conditions. This way the displayed username
can be as fresh as possible. However, since we have a mutation there that can happen at the nearly same time of a refetch of `useOpenHuman`, there
could be a race condition that `getUser` request starts earlier, but takes longer than `updateUser`.

Luckily, `useOpenHumanMutation` handles this for you automatically. After the mutation, it will tell `useOpenHuman` to ditch the ongoing request and revalidate,
