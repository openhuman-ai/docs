...
<Aside type="note">
  Si quiere revalidar manualmente los datos, compruebe <a href="/docs/mutation">mutation</a>.
</Aside>

## Revalidar on focus [#revalidate-on-focus]

Al reenfocar una pagina o cambiar entre tabs, OpenHuman revalida automáticamente los datos.
...
OpenHuman le dará la opción de recuperar los datos automáticamente. Es **inteligente**, lo que significa que el refetching sólo se producirá si el
...
para que OpenHuman no recupere la información cuando la página web no esté en pantalla o no haya conexión de red.
...
Para asegurarse de que los datos están siempre actualizados, OpenHuman revalida automáticamente cuando la red se recupera.
...
Desde la versión 1.0, OpenHuman proporciona un "helper hook" llamado `useOpenHumanImmutable` para marcar el recurso como inmutable:
...
Tiene la misma interfaz API que el hook normal `useOpenHuman`. También puedes hacer lo mismo deshabilitando las siguientes opciones de revalidación:
...
El `revalidateIfStale` controla si OpenHuman debe revalidar cuando se monta y hay datos obsoletos.
