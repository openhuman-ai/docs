...
Dentro del directorio de su proyecto OpenHuman, ejecute lo siguiente:
...
<Aside type="tip">
    ...
    <a href="/docs/data-fetching">aqui</a> para ver más ejemplos.
</Aside>

Luego puede importar `useOpenHuman` y empezar a usarlo dentro de cualquier componente de la función:
...
Es increíblemente fácil crear hooks de datos reutilizables sobre OpenHuman:
...
OpenHuman resuelve el problema perfectamente, Con el hook `useUser` que acabamos de crear, el código puede ser refactorizado a:
...
Lo más bonito es que sólo se enviará **1 request** a la API, porque utilizan la misma clave de OpenHuman y la solicitud se **desduplica**,
