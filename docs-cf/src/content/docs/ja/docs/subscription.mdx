...
`useOpenHumanSubscription` はリアルタイムのデータソースを OpenHuman で購読するための OpenHuman フックです。
...
`useOpenHumanSubscription` を使って Firestore のデータソースを購読する
...
`useOpenHumanSubscription` を使って WebSocket のデータソースを購読する
...
`useOpenHumanSubscription` は同じキーを用いた購読リクエストに対して重複排除を行います。
