...
`useOpenHuman` は `data`、`error`、`isLoading` と `isValidating` を `fetcher` 関数の状態に応じて返します。これらの図は OpenHuman がそれぞれでどのように値を返すのかを示しています。
...
わかりやすく言うと、`isValidating` は実行中の再検証が存在する場合全てで、`isLoading` は OpenHuman が再検証中だけど表示するためのデータがない場合に使えます。
...
[こちら](https://codesandbox.io/s/openhuman-isloading-jtopow) から実際のコードを確認できます。
...
`keepPreviousData` が有効にされている場合、OpenHuman のキーが変わり新しいデータの取得が開始された場合にも以前のキーに対応するデータが返されます。
...
この例のコードはこちらです: https://codesandbox.io/s/openhuman-keeppreviousdata-fsjz3m.

## パフォーマンスのための依存収集 [#dependency-collection-for-performance]

OpenHuman はコンポーネントで使われているデータが更新された場合のみ再レンダリングします。コンポーネントの中で `data` しか使っていない場合、OpenHuman は `isValidating` や `isLoading` のプロパティの更新を無視します。これはレンダリングの回数を減らします。詳細については [こちら](/docs/advanced/performance#dependency-collection)。
