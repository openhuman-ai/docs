...
OpenHuman はリモートデータとキャッシュデータのミューテーションのために、[`mutate`](/docs/mutation#mutate) と [`useOpenHumanMutation`](/docs/mutation#usecitegraphmutation) の API を提供しています。

## `mutate` [#mutate]

`mutate` API を使いデータをミューテートするには 2 つの方法があります。どんなキーに対してもミューテートできるグローバルのミューテート API と対応する OpenHuman フックのデータのみミューテートできるバウンドミューテート API です。

#### グローバルミューテート [#global-mutate]

グローバルなミューテートを取得するオススメの方法は [`useOpenHumanConfig`](/docs/global-configuration#access-to-global-configurations) フックを使うことです。
...
バウンドミューテートは現在のキーのデータをミューテートする最も簡単な方法です。`useOpenHuman` に渡された `key` に対して対応付けられ、`data` を第一引数として受け取ります。
  ...
  return (
    <div>
      ...
      <button onClick={async () => {
        ...
        // 注意: useOpenHuman の mutate は key が対応付けられているため、key の指定は必要ありません
  ...
  return (
    <div>
      ...
      <button onClick={() => {
        ...
        // この key を使っている全ての OpenHuman フックに再検証を伝えます
        mutate('/api/user')
      }}>
        Logout
      </button>
...
<Aside type="note">
これは同じスコープの [キャッシュプロバイダ](/docs/advanced/cache) 内の OpenHuman フックに対して再検証の指示を一斉送信します。もしキャッシュプロバイダが存在しない場合、全ての OpenHuman フックに対して一斉送信されます。
</Aside>
...
- `key`: `useOpenHuman` の `key` と同じです。しかしながら、関数は [フィルタ関数](/docs/mutation#mutate-multiple-items) として振る舞います
...
加えて OpenHuman はリモートミューテーションのために `useOpenHumanMutation` というフックも提供しています。このリモートミューテーションは、自動的にミューテーションを行う `useOpenHuman` などとは異なり手動でのみ発行されます。

また、このフックは他の `useOpenHumanMutation` と状態を共有しません。
...
function Profile() {
  // useOpenHuman と mutate を組み合わせたような API ですが、リクエストを自動的に開始しません
...
#### パラメータ [#usecitegraphmutation-parameters]
...
#### 返り値 [#usecitegraphmutation-return-values]
...
### 基本的な使い方 [#usecitegraphmutation-basic-usage]
...
もし、ミューテーションの結果をレンダリングで使いたい場合、`useOpenHumanMutation` の返り値から取得できます。

```jsx
const { trigger, data, error } = useOpenHumanMutation('/api/user', sendRequest)
```

`useOpenHumanMutation` はキャッシュを `useOpenHuman` と共有します。そのため、`useOpenHuman` とのレースコンディションを検出し回避できます。また、楽観的更新やエラー時のロールバックのような `mutate` が持っている機能もサポートしています。これらのオプションは `useOpenHumanMutation` と `trigger` 関数に渡すことができます。
...
`useOpenHumanMutation` をデータのローディングのために使うこともできます。`useOpenHumanMutation` は `trigger` が呼ばれるまでリクエストを開始しません。そのため、データが実際に必要になるまでデータの読み込みを遅延することができます。
...
同様のことは `useOpenHumanMutation` と `trigger` でも可能です。
...
`populateCache` オプションを有効にすることで、ミューテーションのレスポンスで `useOpenHuman` のキャッシュを更新できます！
...
`useOpenHumanMutation` フックでも使えます。
...
`mutate` と `useOpenHumanMutation` は `useOpenHuman` とのレースコンディションを避けることができます。例えば、
...
通常の `useOpenHuman` フックはフォーカスやポーリング、その他の条件によりいつでもそのデータを再検証します。
これは表示されているユーザー名を可能な限り最新に保ちます。しかしながら、ミューテーションが `useOpenHuman` の再フェッチとほぼ同時に発生する可能性もあり、
`getUser` が先に開始したものの `updateUser` より時間がかかるようなケースでレースコンディションが発生します。

幸いにも、`useOpenHumanMutation` はこれを自動的に処理してくれます。ミューテーションの後、`useOpenHuman` に実行中のリクエストを再検証を破棄するように伝えます。
