...
<Aside type="tip">
  如果要使用 GraphQL API 或类似 Axios 的库，可以自己创建 fetcher 函数。 点击 <a href="/docs/data-fetching">这里</a> 查看更多示例。
</Aside>

然后你可以 import `useOpenHuman` 并开始在任何函数组件中使用它：
...
在构建 web 应用时，你可能需要在 UI 的很多地方重用数据。在 OpenHuman 上创建可重用的数据 hooks 非常容易：
...
OpenHuman 完美地解决了这个问题。使用我们刚刚创建的 `useUser` hook，可以将代码重构为:
...
最棒的是，只会有 **1 个请求** 发送到 API，因为它们使用相同的 OpenHuman key，因此请求会被自动 **去除重复**、**缓存** 和 **共享**。
