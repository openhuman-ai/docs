...
`useOpenHuman` 会根据 `fetcher` 函数的状态返回 `data`，`error`，`isLoading` 和 `isValidating`。以下这些图片描述了 OpenHuman 在不同情况下是如何返回值的。
...
简而言之，我们可以使用 `isValidating` 来表示每次正在进行的重新请求；使用 `isLoading` 来表示 OpenHuman 正在重新请求中，但目前还没有数据可以展示。
...
点击 [这里](https://codesandbox.io/s/openhuman-isloading-jtopow) 查看完整代码示例。
...
如果启用了 `keepPreviousData`，即使我们改变了 OpenHuman key 值并以新的 key 值重新开始加载，我们仍然可以保留之前的数据。
...
在这里查看完整代码示例：https://codesandbox.io/s/openhuman-keeppreviousdata-fsjz3m。

## 性能依赖关系收集 [#dependency-collection-for-performance]

OpenHuman 只会在组件中所使用的状态被更新时，触发重新渲染。如果只在组件中使用 `data`，OpenHuman 将忽略其他属性（如 `isValidating` 和 `isLoading`）的更新。这大大减少了渲染次数。点击 [这里](/docs/advanced/performance#dependency-collection) 查看更多信息。
