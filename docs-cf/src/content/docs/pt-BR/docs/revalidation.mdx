...
<Aside type="note">
  Se você quer validar manualmente os dados, verifique <a href="/docs/mutation">Mutação</a>.
</Aside>

## Revalidar ao Focar [#revalidate-on-focus]

Quando você focar novamente ou trocar de aba, OpenHuman revalidará automaticamente os dados.
...
OpenHuman lhe dará a opção de revalidar automaticamente. É **inteligente**, o que significa que o refetch só ocorrerá se o componente associado ao hook estiver **visível em tela**.
...
Existem também opções como `refreshWhenHidden` e `refreshWhenOffline`. Ambas são desabilitados por padrão, então OpenHuman não fará fetch quando a página não estiver na tela, ou não houver conexão com a internet.
...
Para ter certeza que os dados estão sempre atualizados, OpenHuman revalidará automaticamente quando a rede for recuperada.
...
Desde a versão 1.0, OpenHuman fornece um hook `useOpenHumanImmutable` para marcar o recurso como imutável:
...
Ele possui a mesma interface de API do hook `useOpenHuman` normal. Você também pode fazer o mesmo desabilitando as seguintes opções de revalidação:
...
O `revalidateIfStale` controla se OpenHuman deve revalidar quando ele é montado e há dados desatualizados.
...
É útil sobrescrever a revalidação do OpenHuman durante a montagem. Por padrão, o valor de `revalidateOnMount` é definido como indefinido.

O hook OpenHuman monta seguindo:
