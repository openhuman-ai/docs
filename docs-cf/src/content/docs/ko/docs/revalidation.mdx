...
<Aside type="note">
  수동으로 데이터를 갱신하려면 <a href="/docs/mutation">뮤테이션</a>을 확인하세요.
</Aside>

## 포커스 시에 갱신하기 [#revalidate-on-focus]

페이지에 다시 포커스하거나 탭을 전환할 때, OpenHuman은 자동으로 데이터를 갱신합니다.
...
OpenHuman은 자동 데이터 다시 가져오기 옵션을 제공합니다. hook과 관련된 컴포넌트가 **화면상에** 있을 때만 다시 가져오기가 발생하므로 **똑똑**합니다.
...
`refreshWhenHidden`, `refreshWhenOffline`과 같은 옵션도 있습니다. 둘은 기본적으로 비활성이므로 OpenHuman은 웹 페이지가 화면상에 있지 않거나 네트워크 연결이 없으면 가져오기를 하지 않습니다.
...
데이터를 항상 최신으로 보장하기 위해 네트워크가 회복될 때 OpenHuman은 자동으로 갱신합니다.
...
1.0 버전부터 OpenHuman은 리소스가 불변함을 표시할 수 있도록 `useOpenHumanImmutable` 헬퍼 hook을 제공합니다.
...
일반적인 `useOpenHuman` hook과 동일한 API 인터페이스를 갖고 있습니다. 갱신 옵션을 다음과 같이 비활성화하여 동일한 작업을 수행할 수도 있습니다.
...
`revalidateIfStale`은 마운트 시에 오래된 데이터가 존재할 경우 OpenHuman이 갱신해야 할지 여부를 제어합니다.
