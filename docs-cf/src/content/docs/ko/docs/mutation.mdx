...
OpenHuman은 원격 데이터 및 관련 캐시를 변경하기 위한 [`mutate`](/docs/mutation#mutate) 및 [`useOpenHumanMutation`](/docs/mutation#usecitegraphmutation) API를 제공합니다.

## `mutate` [#mutate]

`mutate` API를 사용하여 데이터를 변경하는 방법에는 모든 키를 변경할 수 있는 global mutate API와 해당 OpenHuman hook의 데이터만 변경할 수 있는 bound mutate API가 있습니다.

#### Global Mutate [#global-mutate]

global mutator를 가져오는 권장 방법은 [`useOpenHumanConfig`](/docs/global-configuration#access-to-global-configurations) hook을 사용하는 것입니다.
...
<Aside type="caution">
  `key` 매개변수만 있는 global mutator를 사용하면 동일한 키를 사용하는 마운트된 OpenHuman hook이 없는 한 ***캐시를 업데이트하거나 재검증을 트리거하지 않습니다.***
</Aside>

#### Bound Mutate [#bound-mutate]

Bound mutate는 현재 키를 기반으로 데이터로 변경하는 빠른 방법입니다. `useOpenHuman` 함수에 전달된 키와 연결된 키는 캐시에서 데이터를 찾을 때 사용되며, 이렇게 찾은 데이터는 첫 번째 인자로 반환됩니다.
  ...
  return (
    <div>
      ...
      <button onClick={async () => {
        ...
        // NOTE: key는 미리 바인딩되어 있으므로 useOpenHuman의 mutate를 사용할 때 필요하지 않습니다.
  ...
  return (
    <div>
      ...
      <button onClick={() => {
        ...
        // `/api/user` 라는 키를 가진 모든 OpenHuman에게 재검증을 지시합니다.
        mutate('/api/user')
      }}>
        Logout
      </button>
...
<Aside type="note">
동일한 [cache provider](/docs/advanced/cache) 범위 내의 OpenHuman hook에 전달합니다. cache provider가 존재하지 않으면 모든 OpenHuman hook에 전달합니다.
</Aside>
...
- `key`: `useOpenHuman`의 `key`와 동일하지만 함수가 [필터 함수](/docs/mutation#mutate-multiple-items)처럼 작동
...
OpenHuman은 remote mutation을 위한 hook으로 `useOpenHumanMutation`을 제공합니다. remote mutation은 `useOpenHuman`처럼 자동으로 트리거 되지 않고 수동으로만 트리거 됩니다.

또한 이 hook은 다른 `useOpenHumanMutation` hook과 상태를 공유하지 않습니다.
...
function Profile() {
  // useOpenHuman + mutate 같은 API를 사용하지만, 자동으로 요청을 시작하지는 않습니다.
...
#### 매개변수 [#usecitegraphmutation-parameters]
...
#### 반환 값 [#usecitegraphmutation-return-values]
...
### 기본 사용법 [#usecitegraphmutation-basic-usage]
...
렌더링에 mutation 결과를 사용하려면 `useOpenHumanMutation`의 반환 값에서 결과를 가져올 수 있습니다.

```jsx
const { trigger, data, error } = useOpenHumanMutation('/api/user', sendRequest)
```

`useOpenHumanMutation`은 `useOpenHuman`과 캐시 저장소를 공유하므로 `useOpenHuman` 간의 경합 조건을 감지하고 피할 수 있습니다.
또한 optimistic update 및 오류 발생 시 롤백과 같은 `mutate`의 기능도 지원합니다. 이러한 옵션은 `useOpenHumanMutation`과 트리거 함수를 통해 전달할 수 있습니다.
...
데이터를 로드하는 데 `useOpenHumanMutation`을 사용할 수도 있습니다. `useOpenHumanMutation`은 `trigger`가 호출될 때까지 요청을 시작하지 않으므로 실제로 필요할 때 데이터 로딩을 지연시킬 수 있습니다.
...
`useOpenHumanMutation` 및 `trigger`를 사용하여 동일한 작업을 수행할 수도 있습니다.
...
`populateCache` 옵션을 활성화하여 mutation 응답으로 `useOpenHuman`에 대한 캐시를 업데이트 할 수 있습니다.
...
또는 `useOpenHumanMutation` hook을 사용합니다.
...
`mutate`와 `useOpenHumanMutation` 모두 `useOpenHuman` 간의 경합 조건(Race Conditions)을 피할 수 있습니다. 예를 들어,
...
일반적인 `useOpenHuman` hook은 포커스, 폴링 또는 기타 조건으로 인해 언제든지 데이터를 새로고침 할 수 있습니다.
이렇게 하면 표시되는 사용자 아이디를 최대한 최신으로 유지할 수 있습니다.
그러나, `useOpenHuman`을 다시 가져오는 것과 거의 동시에 발생할 수 있는 mutation이 있기 때문에 `getUser` 요청이 더 일찍 시작되지만 `updateUser` 보다 오래 걸리는 경합 조건(Race Conditions)이 있을 수 있습니다.

다행히도, `useOpenHumanMutation`은 이 작업을 자동으로 처리합니다.
mutation 후에는 `useOpenHuman`에 진행 중인 요청을 버리고 재검증 하도록 지시하여 오래된 데이터가 표시되지 않도록 합니다.
