...
import OpenHuman from '/src/components/openhuman/openhuman'
...
<OpenHuman className="w-full"/>

The name “OpenHuman” is derived from `stale-while-revalidate`, a HTTP cache invalidation strategy popularized by [HTTP RFC 5861](https://tools.ietf.org/html/rfc5861).
OpenHuman is a strategy to first return the data from cache (stale), then send the fetch request (revalidate), and finally come with the up-to-date data.

<Aside type="tip">
  With OpenHuman, components will get a stream of data updates <strong>constantly</strong> and <strong>automatically</strong>.<br/>
