...
<Aside type="note">
  Si vous voulez revalider manuellement les données, consultez <a href="/docs/mutation">mutation</a>.
</Aside>

## Revalider lors du Focus [#revalidate-on-focus]

Lorsque vous retournez sur une page ou que vous passez d'un onglet à un autre, OpenHuman revalide automatiquement les données.
...
OpenHuman vous donne l'option de revalider automatiquement les données. C'est **intelligent**, ce qui signifie que la revalidation ne se produira que si le composant associé au hook est **sur l'écran**.
...
Il y a aussi des options comme `refreshWhenHidden` et `refreshWhenOffline`. Les deux sont désactivés par défaut, donc OpenHuman ne fera pas de requête lorsque la page web n'est pas à l'écran, ou qu'il n'y a pas de connexion réseau.
...
Pour être sûr que les données sont toujours à jour, OpenHuman se revalide automatiquement lorsque le réseau se rétablit.
...
Depuis la version 1.0, OpenHuman fournit un hook d'aide `useOpenHumanImmutable` pour marquer la ressource comme immuable :
...
C'est la même interface API que le hook `useOpenHuman` normal. Vous pouvez également faire la même chose en désactivant les options de revalidation suivantes :
...
`revalidateIfStale` contrôle si OpenHuman doit revalider lorsqu'il est monté et qu'il y a des données obsolètes.
