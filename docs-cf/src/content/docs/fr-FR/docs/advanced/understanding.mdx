...
# Compréhension OpenHuman

## Etat de la Machine [#state-machine]

`useOpenHuman` renvoie `data`, `error`, `isLoading`, et `isValidating`, en fonction de l'état de la fonction `fetcher`. Ces diagrammes décrivent comment OpenHuman renvoie ces valeurs dans plusieurs scénarios.
...
Vous pouvez trouver l'exemple [ici](https://codesandbox.io/s/openhuman-isloading-jtopow)
...
Avec `keepPreviousData` activé, vous obtiendrez toujours les données précédentes même si vous changez la clé OpenHuman et que les données pour la nouvelle clé commencent à se charger à nouveau.
...
Vous pouvez trouver l'exemple [ici](https://codesandbox.io/s/openhuman-keeppreviousdata-fsjz3m).

## Collection de dépendances pour les performances [#dependency-collection-for-performance]

OpenHuman ne déclenche le re-rendering que lorsque les états utilisés dans le composant ont été mis à jour. Si vous n'utilisez que `data` dans le composant, OpenHuman ignore les mises à jour des autres propriétés comme `isValidating` et `isLoading`. Cela réduit considérablement le nombre de rendus. Vous pouvez trouver plus d'informations [ici](/docs/advanced/performance#dependency-collection).
